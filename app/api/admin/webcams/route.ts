...
// GET - List all webcams or get single webcam
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to create webcam' }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to update webcam' }, { status: 500 });
  }
}

// DELETE - Delete webcam
export async function DELETE(request: NextRequest) {
  ...
  try {
    const { data: webcam } = await supabase
      .from('webcams')
      .select('name')
      .eq('id', id)
      .single();
