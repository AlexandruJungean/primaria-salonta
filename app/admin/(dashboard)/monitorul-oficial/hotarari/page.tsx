...
type TabType = 'registru_hotarari' | 'registru_proiecte';

const TABS: { key: TabType; label: string; icon: typeof BookOpen }[] = [
  { key: 'registru_hotarari', label: 'Registre Hotărâri Adoptate', icon: BookOpen },
  { key: 'registru_proiecte', label: 'Registre Proiecte de Hotărâri', icon: ClipboardList },
];

export default function HotarariMolAdminPage() {
  const router = useRouter();
  
  // Tab state
  const [activeTab, setActiveTab] = useState<TabType>('registru_hotarari');
  ...
  const loadDocuments = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        source_folder: 'hotararile-autoritatii-deliberative',
  ...
  const handleEditDocument = (doc: Document) => {
    router.push(`/admin/monitorul-oficial/hotarari/${doc.id}`);
  };
  ...
  const documentColumns = [
    {
      ...
      render: (item: Document) => (
        <div>
          ...
          {item.year && (
            <span className="text-xs text-slate-500">Anul {item.year}</span>
          )}
        </div>
      ),
    },
  ...
  return (
    <div>
      <AdminPageHeader
        title="Registre Hotărâri (Monitorul Oficial)"
        breadcrumbs={[
      ...
      <AdminCard className="mb-6">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <AdminButton 
            icon={Plus} 
            onClick={() => router.push(`/admin/monitorul-oficial/hotarari/nou?subcategory=${activeTab}`)}
          >
            Încarcă Document
          </AdminButton>
        </div>
      ...
      <AdminTable
        ...
        emptyMessage={`Nu există ${currentTabInfo.label.toLowerCase()}. Apasă 'Încarcă Document' pentru a adăuga.`}
      />
