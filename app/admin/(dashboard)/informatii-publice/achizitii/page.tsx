...
type TabType = 'documente' | 'formulare';

const TABS: { key: TabType; label: string; icon: typeof ShoppingCart }[] = [
  { key: 'documente', label: 'Documente Achiziții', icon: ShoppingCart },
  { key: 'formulare', label: 'Formulare necesare', icon: ClipboardList },
];

export default function AchizitiiAdminPage() {
  const router = useRouter();
  
  // Tab state
  const [activeTab, setActiveTab] = useState<TabType>('documente');
  ...
  const handleEditDocument = (doc: Document) => {
    router.push(`/admin/informatii-publice/achizitii/${doc.id}`);
  };
    ...
    <div>
      ...
      <AdminCard className="mb-6">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <AdminButton 
            icon={Plus} 
            onClick={() => router.push(`/admin/informatii-publice/achizitii/nou?subcategory=${activeTab}`)}
          >
            Încarcă Document
          </AdminButton>
        </div>
