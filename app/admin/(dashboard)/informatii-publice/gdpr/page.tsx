...
export default function GdprPage() {
  ...
  const [sections, setSections] = useState<GdprSection[]>([]);
  const [expandedId, setExpandedId] = useState<string | null>(null);
  const [deleteDoc, setDeleteDoc] = useState<GdprDocument | null>(null);
  const [deleting, setDeleting] = useState(false);

  const fetchDocuments = useCallback(async () => {
    try {
      const response = await adminFetch('/api/admin/documents?category=gdpr&limit=500');
      ...
      if (formDocs.length > 0) {
        sectionsData.push({
          id: 'cereri',
          label: 'Formulare și Cereri GDPR',
  ...
  if (loading) {
    return (
      <div>
        <AdminPageHeader
          title="GDPR"
          description="Gestionează documentele de protecție a datelor"
          breadcrumbs={[
  ...
  return (
    <div>
      <AdminPageHeader
        title="GDPR"
        description={`${totalDocuments} documente în ${sections.length} secțiuni`}
        breadcrumbs={[
          { label: 'Informații Publice' },
          { label: 'GDPR' },
        ]}
        actions={
          <AdminButton
            icon={Plus}
            onClick={() => router.push('/admin/informatii-publice/gdpr/nou')}
          >
            Adaugă Document
          </AdminButton>
        }
      ...
      {sections.length === 0 ? (
        <AdminCard>
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-4">Nu există documente GDPR</p>
            <AdminButton
              icon={Plus}
              onClick={() => router.push('/admin/informatii-publice/gdpr/nou')}
            >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <AdminButton
                          ...
                          size="sm"
                          icon={Edit}
                          onClick={() => router.push(`/admin/informatii-publice/gdpr/${doc.id}`)}
                        >
                          Editează
                        </AdminButton>
                        <button
