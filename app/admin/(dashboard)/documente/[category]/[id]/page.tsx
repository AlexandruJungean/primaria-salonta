...
/**
 * This page redirects to the new document edit structure.
 * Kept for backward compatibility with existing bookmarks/links.
 */
export default function DocumentEditRedirectPage() {
  ...
  useEffect(() => {
    ...
    const url = `/admin/documente/gestiune/${id}?type=category&value=${encodeURIComponent(category)}&label=${encodeURIComponent(label)}`;
    router.replace(url);
  }, [category, id, router]);
