...
function DocumenteGestiuneContent() {
  ...
  const loadFilters = async () => {
    // Build the base query for years and subcategories
  ...
  const handleEdit = (item: Document) => {
    ...
    router.push(`/admin/documente/gestiune/${item.id}?${params.toString()}`);
  };
      ...
      render: (item: Document) => (
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-slate-500 truncate">{item.file_name} • {formatFileSize(item.file_size)}</p>
          </div>
        </div>
      ),
    },
        ...
        actions={
          <AdminButton 
            ...
            onClick={() => {
              ...
              router.push(`/admin/documente/gestiune/nou?${params.toString()}`);
            }}
          >
...
export default function DocumenteGestiunePage() {
  return (
    <Suspense fallback={
